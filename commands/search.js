...
module.exports = {
  name: "search",
  description: "Hiển thị các kết quả tìm kiếm.",
  usage: "[Tên]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Dùng - **\`${GuildDB.prefix}search [Tên]\``
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      let msg = await message.channel.send(
        "**Nhập số của kết quả bạn muốn phát ! Bạn có `30 giây`.**"
      );
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "❌ | **Bạn mất khá nhiều thời gian để trả lời. Nếu muốn phát kết quả, chạy lại lệnh !**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(message.channel, "Hãy trả lời đúng `ID`.");
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song) return client.sendTime(message.channel, "Không tìm thấy.");
