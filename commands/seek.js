...
module.exports = {
    name: "seek",
    description: "Di chuyển đoạn nhạc đến vị trí khác.",
    usage: "[3h 2m 1s]",
    permissions: {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!player.queue.current.isSeekable) return client.sendTime(message.channel, "❌ | **Không tìm thấy đoạn nhạc đấy !**");
        let SeekTo = client.ParseHumanTime(args.join(" "));
        if (!SeekTo) return client.sendTime(message.channel, `**Dùng - **\`${GuildDB.prefix}seek [h m s]\` \n**Ví dụ - **\`${GuildDB.prefix}seek 2m 10s\``);
