...
module.exports = {
    name: "volume",
    description: "Kiểm tra và thay đổi âm lượng.",
    usage: "[Số]",
    permissions: {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!args[0]) return client.sendTime(message.channel, `🔉 | Âm lượng là \`${player.volume}\`.`);
        ...
        if (!parseInt(args[0])) return client.sendTime(message.channel, `**Hãy chọn số bạn muốn đặt âm lượng.**`);
        let vol = parseInt(args[0]);
        player.setVolume(vol);
        client.sendTime(message.channel, `🔉 | **Âm lượng được đặt thành** \`${player.volume}\``);
    },
    SlashCommand: {
        options: [
            {
