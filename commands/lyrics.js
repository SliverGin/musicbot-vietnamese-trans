...
module.exports = {
  name: "lyrics",
  description: "Hiển thị lời bài hát bạn tìm kiếm (tham khảo).",
  usage: "[Tên Nhạc]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0] && !player) return client.sendTime(message.channel, "❌ | **Bạn phải chơi nhạc để dùng lệnh**");
    if (!args[0]) SongTitle = player.queue.current.title;

    let lyrics = await lyricsFinder(SongTitle);
    if (!lyrics) return client.sendTime(message.channel, `**Không tìm thấy lời bài hát -** \`${SongTitle}\``);
    lyrics = lyrics.split("\n"); //spliting into lines
