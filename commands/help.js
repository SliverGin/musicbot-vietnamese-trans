...
module.exports = {
  name: "help",
  description: "Thông tin về BOT và danh sách lệnh.",
  usage: "[Lệnh]",
  permissions: {
   ...
   */
   run: async (client, message, args, { GuildDB }) => {
    ...
    let Embed = new MessageEmbed()
            ...
            .setColor("RANDOM")
            .setFooter(
              `Để biết rõ hơn về từng lệnh, nhập ${
                GuildDB ? GuildDB.prefix : client.config.DefaultPrefix
              }help [Lệnh] | Một ngày tốt lành !`
            ).setDescription(`${Commands.join("\n")}
            
  Hoặc tìm <@852800375266607155> để được trợ giúp.
  [✨ Hỗ trợ](${
    client.config.SupportServer
  })  | Từ <@852800375266607155>`);
    if (!args[0]) message.channel.send(Embed);
      ...
      if (!cmd)
        return client.sendTime(message.channel, `❌ | Không tìm thấy lệnh.`);

      let embed = new MessageEmbed()
        .setAuthor(`Lệnh : ${cmd.name}`, client.config.IconURL)
        .setDescription(cmd.description)
        .setColor("GREEN")
        //.addField("Name", cmd.name, true)
        .addField("Biệt lệnh", `\`${cmd.aliases.join(", ")}\``, true)
        .addField(
          "Cách dùng",
          `\`${GuildDB ? GuildDB.prefix : client.config.DefaultPrefix}${
            cmd.name
