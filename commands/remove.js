...
const { TrackUtils } = require("erela.js");
  ...
  module.exports = {
    name: "remove",
    description: `Loại bỏ bài hát ra khỏi hàng đợi.`,
    usage: "[Số]",
    permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!player.queue || !player.queue.length || player.queue.length === 0)
      return message.channel.send("Không có gì trong hàng đợi !");
    let rm = new MessageEmbed()
      .setDescription(`✅ **|** Đã bỏ **\`${Number(args[0])}\`** từ hàng đợi !`)
      .setColor("GREEN")
      if (isNaN(args[0]))rm.setDescription(`**Dùng - **${client.config.prefix}\`remove [Số]\``);
      if (args[0] > player.queue.length)
      rm.setDescription(`Hàng đợi chỉ có ${player.queue.length} bài !`);
